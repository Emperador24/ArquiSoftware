...
La Tabla \ref{tab:rnf} especifica, para cada atributo de calidad priorizado en
\textit{ArchitecturalProposal.tex} (incluidos los cuatro incorporados al revisar las clases 6 a 14
del curso: Integrabilidad, Desplegabilidad, Comprobabilidad y Seguridad física), un requisito no
funcional medible — no una aspiración genérica ("el sistema debe ser rápido"), sino un umbral
verificable. Estos RNF son la base de la que se derivan los escenarios ASR de la Sección
\ref{sec:asr}.

\begin{center}
\begin{longtable}{|p{1.3cm}|p{4.6cm}|p{2.4cm}|p{5.3cm}|}
\hline
\textbf{ID} & \textbf{Requisito no funcional} & \textbf{Atributo} & \textbf{Métrica / umbral}\\
\hline
\endhead
RNF-01 & Validación de una entrada en el ingreso al recinto & Rendimiento & $\leq$ 500 ms por
escaneo, percentil 95.\\
\hline
RNF-02 & Respuesta durante la apertura de venta de un evento masivo & Rendimiento /
Disponibilidad & $\geq$ 2000 solicitudes/segundo sostenidas sin degradar el tiempo de respuesta
del RNF-01.\\
\hline
RNF-03 & Recuperación ante la caída de una instancia de validación de QR & Disponibilidad &
\textit{Failover} $\leq$ 10 s, sin pérdida de transacciones en curso.\\
\hline
RNF-04 & Disponibilidad general del backend & Disponibilidad & $\geq$ 99.9\% mensual (m\'aximo
$\sim$43 min de \textit{downtime} no programado al mes).\\
\hline
RNF-05 & Autenticación y transporte de datos sensibles & Seguridad & Tokens JWT con expiración
$\leq$ 1 h; HTTPS/TLS obligatorio en todas las interfaces; contraseñas nunca en texto plano.\\
\hline
RNF-06 & Bloqueo de doble venta en el mercado secundario & Consistencia & 0\% de ventas
duplicadas de la misma entrada bajo compradores concurrentes (verificado en el PoC de bloqueo,
ver \textit{GuiaEntrega1.md}).\\
\hline
RNF-07 & Actualización de aforo y estado de zonas & Tiempo real & Latencia $\leq$ 3 s entre el
registro de un ingreso/incidente y su reflejo en los tableros de monitoreo.\\
\hline
RNF-08 & Escalado del servicio de Entradas ante un pico de tráfico & Escalabilidad & Nuevas
instancias activas en $\leq$ 2 min desde que se supera el 80\% de uso de CPU/memoria.\\
\hline
RNF-09 & Auditoría de operaciones sensibles & Trazabilidad & 100\% de las transferencias, pagos,
reembolsos y cambios administrativos quedan registrados con usuario, fecha y resultado.\\
\hline
RNF-10 & Uso de la interfaz sin capacitación previa & Usabilidad & Un usuario nuevo completa su
tarea principal (comprar una entrada, registrar asistencia) en $\leq$ 5 pasos, sin ayuda externa.\\
\hline
RNF-11 & Independencia de despliegue entre dominios & Mantenibilidad & Un cambio en un
microservicio se despliega sin requerir cambios ni redespliegue de los demás (ASR-09).\\
\hline
RNF-12 & Consumo uniforme del backend & Portabilidad & Toda funcionalidad expuesta por el API
Gateway es consumible, sin duplicar lógica, desde el portal web y desde la app móvil (ASR-10).\\
\hline
RNF-13 & Aislamiento de integraciones externas & Integrabilidad & Un fallo o cambio en la
Pasarela de Pagos o el Proveedor de Notificaciones no requiere cambios en más de un componente
(el adaptador correspondiente).\\
\hline
RNF-14 & Actualización de un servicio sin interrumpir un evento en curso & Desplegabilidad &
0 s de \textit{downtime} perceptible por el cliente durante un despliegue Blue/Green en horario
de evento.\\
\hline
RNF-15 & Aislamiento de pruebas de un microservicio & Comprobabilidad & Las pruebas de un
microservicio corren sin depender de que los demás microservicios ni los servicios externos
estén activos (mocks/inyección de dependencias).\\
\hline
RNF-16 & Control de aforo máximo por zona & Seguridad física (\textit{Safety}) & El sistema
rechaza el 100\% de los registros de ingreso que excedan el aforo configurado de una zona, sin
excepción por estado de pago.\\
\hline
\caption{Requisitos No Funcionales del sistema, uno o más por atributo de calidad priorizado.}
\label{tab:rnf}
\end{longtable}
\end{center}
...
identificados en la bitácora arquitectónica. Cada escenario se prioriza con el par
\textbf{(Importancia, Dificultad)} en escala Alta/Media/Baja, siguiendo el formato de Árbol de
Utilidad visto en la Clase 5 (ej. (H,H) = alto impacto de negocio y alta incertidumbre/dificultad
técnica) — no una sola columna de prioridad genérica.

\begin{center}
\begin{longtable}{|p{1.1cm}|p{2cm}|p{1.8cm}|p{1.7cm}|p{1.8cm}|p{3.6cm}|p{1.1cm}|p{1.1cm}|}
\hline
\textbf{ID ASR} & \textbf{Título} & \textbf{Atributo} & \textbf{Sub-atributo} &
\textbf{Funcionalidades} & \textbf{Escenario} & \textbf{Import.} & \textbf{Dific.}\\
\hline
\endhead
...
perceptible para el asistente. & Alta & Alta\\
\hline
...
por el API Gateway y queda registrado para auditoría. & Alta & Media\\
\hline
...
tiempos cortos y estables. & Alta & Alta\\
\hline
...
baja latencia tras el registro de un ingreso o un incidente. & Media & Media\\
\hline
...
consultable en un reporte de auditoría. & Media & Baja\\
\hline
...
correspondiente sin capacitación previa. & Baja & Baja\\
\hline
...
sin requerir cambios ni redespliegue del servicio de Entradas. & Media & Baja\\
\hline
...
duplicar lógica de negocio. & Baja & Baja\\
\hline
ASR-11 & Aislamiento de integraciones externas & Integrabilidad & Adaptación de interfaces &
CU-001, CU-003, CU-010 & Un cambio de proveedor de pagos o de notificaciones se absorbe en el
adaptador correspondiente, sin modificar la lógica de negocio de los microservicios que lo usan. &
Media & Media\\
\hline
ASR-12 & Actualización sin interrupción durante un evento en curso & Desplegabilidad & Reemplazo
de servicios & CU-002, CU-006 & Un hotfix al servicio de validación de QR se despliega mientras
hay ingreso masivo activo, sin interrupción perceptible ni inconsistencia de versión. & Media &
Alta\\
\hline
\caption{Árbol de Utilidad — primera versión, con prioridad expresada como (Importancia,
Dificultad).}
\label{tab:asr}
\end{longtable}
\end{center}
