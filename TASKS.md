...
| Architectural proposal | Work/ArchitecturalProposal.tex → Submission/ArchitecturalProposal.pdf | submitted (2026-09-06) | — | Compiled, no LaTeX errors. Re-checked 2026-08-20 against the Clase 2/3 exercise (prioritize QA alta/media/baja, define components/connections, explain how the architecture satisfies each QA) — already covers all three explicitly, no changes needed. **2026-08-25**: expanded § "Disponibilidad — Alta" with the Clase 6 (Availability) exercise. **2026-09-06**: added the 4 missing QA rows (Integrabilidad, Desplegabilidad, Comprobabilidad, Safety) and full Bass/Kazman tactics tables for all 8 remaining QA classes (Clases 7-14) — see Bitácora. Recompiled clean (30 pages, up from 17) and re-copied to `Submission/`. Also fixed a process slip the same day: the compiled PDF had briefly ended up committed inside `Work/` alongside the `.tex` — removed (Work/ now keeps only source files, per `CLAUDE.md`), and `Work/*.pdf` added to `.gitignore` to prevent recurrence. |
...
| SAD (arc42 template) — v1 draft | Work/DescripcionArquitecturaSoftware.tex | in-progress (2026-09-06) | 2026-10-09 (tentative, folds into Entrega 1) | **First draft assembled 2026-08-25**, consolidating existing material into the course's arc42-based SAD template: intro, 32-CU functional overview, domain concept table, stakeholders, a first-cut Utility Tree (ASR-01..ASR-10), constraints, all 6 C4 views (context/container/component/process/physical, embedding `Diagrams/*.png`), 4 ADRs derived from the founding decisions already in the Bitácora, a technical-risk table, glossary, and references. **2026-09-06**: added a full Non-Functional Requirements section (RNF-01..RNF-16, one measurable threshold per priority QA), reformatted the Utility Tree to use (Importancia, Dificultad) instead of a single priority column, and added ASR-11/ASR-12 (Integrabilidad, Desplegabilidad) — now 22 pages, compiles clean. `ArchitecturalProposal.tex` grew from 17 to 30 pages the same day: added 4 QA rows that were missing (Integrabilidad, Desplegabilidad, Comprobabilidad, Safety) and wrote full Bass/Kazman tactics/patterns tables (≥2 alternatives compared per scenario) for all 8 remaining QA classes from Clases 6-14 (previously only Disponibilidad had this treatment) — see Bitácora entry 2026-09-06. Still left pending: full domain UML class diagram, component breakdown for the other 3 containers, dev/test deployment diagrams, per-service ER data models, and — new as of 2026-09-06 — deciding backend language (still "por definir"), RabbitMQ vs. Kafka, the mobile-stack ADR-05 (needs a Flutter demo), and the QR strategy left open by PoC-02. Not yet copied to `Submission/` — still a draft. |
| PoCs de desafíos técnicos | Proyecto/App/PoCs/ | in-progress (2026-09-06) | folds into Entrega 1 | **2 of 2-3 PoCs done** (2026-09-06): PoC-01 (bloqueo de concurrencia, confirms ADR-03 with real measured data) and PoC-02 (QR validation strategy, inconclusive result — SQLite-local benchmark didn't reproduce real network latency, decision left open). Detail in `PoCs/README.md` and Bitácora. Recommended 3rd PoC (failover) still pending. |
